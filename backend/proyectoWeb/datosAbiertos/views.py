...
class DatosCoches(View):
    ...
    def get(self, request):
        ...
        response = requests.request("GET", self.url, headers=self.headers, params=parametros).text

        # return response
        return JsonResponse({"ok": True, "coches":response}, safe=False)
...
class DatosGasolineras(View):
    ...
    def __init__(self):
        ...
        distributions = requests.get('http://datos.gob.es/apidata/catalog/distribution/dataset/e05068001-precio-de-carburantes-en-las-gasolineras-espanolas').text
    ...
    def get(self, request):
        ...
        for gasolinera in self.gasolinerasJson['ListaEESSPrecio']:
            ...
            gasolinera['Precio Gasoleo A'] = float(gasolinera['Precio Gasoleo A'].replace(",","."))

        gasolinerasOrdenadas = sorted(self.gasolinerasJson['ListaEESSPrecio'], key=lambda x:x['Precio Gasoleo A'])

        for gasolinera in gasolinerasOrdenadas:
            condicion = True
            precioGasA = gasolinera['Precio Gasoleo A']
            if(precio != None and precio != ""):
                condicion = condicion and precioGasA < float(precio) and precioGasA != -1.0
            if(provincia != None and provincia != ""):
                condicion = condicion and gasolinera['Provincia'].casefold().find(provincia.casefold()) >= 0
            if(municipio != None and municipio != ""):
                condicion = condicion and gasolinera['Municipio'].casefold().find(municipio.casefold()) >= 0
